<?
  ...
  $permlevel = $authdata['permlevel'];
    ...
    $msg = "Рахунки не можуть бути згенерован╕, оск╕льки зараз або проводиться перерахунок тариф╕в, або обробляэться запит детально╖ статистики";
    $msg = str_replace("\n", "<br>", $msg);
    $msg = urlencode($msg);
...
?>
	<tr>
		...
		<td width="80%">
			<table>
			<caption>Генеруються рахунки. Будь ласка, зачекайте...</caption>
			<form name="pbform">
				<tr>
				...
				<tr>
<?
	$msg = "Не вс╕ рахунки було згенеровано.\nДочекайтеся, будь ласка, доки програма створення рахунк╕в зак╕нчить свою роботу";
	$msg = str_replace("\n", "<br>", $msg);
	$msg = urlencode($msg);
	$loc = "show_error.php?error=".$msg;
?>
					<td colspan="2"><a href="<? echo $loc; ?>" name="pblink" target="_new">Переглянути згенерован╕ рахунки</a></td>
				</tr>
				<tr>
					<td colspan="2"><a href="<? echo $loc; ?>" name="pblink_notification" target="_new">Переглянути пов╕домлення</a></td>
				</tr>
			</form>
			</table>
		</td>
...
P.S.
    Файл з рахунком надсила╓ться Вам у формат╕ Adobe Portable Document Format (PDF).
    Щоб переглянути рахунок, Вам необх╕дно мати встановленою програму \"Adobe Reader\",
    яку Ви можете завантажити з нашого FTP серверу за такою адресою
...
?>
				...
				<tr bgcolor="white">
					<th colspan="3" align="right">Вкаж╕ть дату в╕дправлення пов╕домлення</th>
					<td>
...
?>    
        <tr bgcolor="white">
          <td colspan="4"><input type="submit" name="save" value="Сформувати рахунки"></td>
        </tr>
      </table>
    </td>
  </tr>
