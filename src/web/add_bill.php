<?
  ...
  if(!empty($_POST["create"])){
    ...
    add_bill($mysql, $contract_id, $y, $m);
		log_event($mysql, $client_id, "bill", "", "add", "Created invoice for contract id $contract_id, period $y/$m");
		header("Location: show_bill.php?client_id=".$client_id);
		exit(0);
    } // end of "create"
  $query = "select
      id,
...
<? include("left_cl.php"); ?>
      ...
      <td valign="top">
        ...
        <caption>Вибер╕ть Догов╕р для рахунку та пер╕од</caption>
          <tr bgcolor="white">
            <td>
